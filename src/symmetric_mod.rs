...
pub fn encrypt_symmetric(
    ...
    secret_passcode_bytes: &secrecy::SecretVec<u8>,
    ...
    let Ok(ciphertext) =
        aes_gcm::aead::Aead::encrypt(&cipher, &nonce, secret_string.expose_secret().as_bytes())
    else {
        return None;
    };
...
pub fn decrypt_symmetric(
    ...
    let encrypted_bytes =
        <base64ct::Base64 as base64ct::Encoding>::decode_vec(&encrypted_string.0).unwrap();
    //only first 32 bytes
    let mut secret_passcode_32bytes = [0u8; 32];
    secret_passcode_32bytes.copy_from_slice(&secret_passcode_bytes.expose_secret()[0..32]);
...
fn password_to_passcode(secret_password: &str) -> secrecy::SecretVec<u8> {
    ...
    Argon2::default()
        .hash_password_into(secret_password.as_bytes(), salt, &mut secret_passcode)
        .unwrap();
...
pub fn encrypt_symmetric_with_password(
    secret_string: &secrecy::SecretString,
    secret_password: &secrecy::SecretString,
) -> Option<EncryptedString> {
...
///
/// Returns the secret_string
pub fn decrypt_symmetric_with_password(
    encrypted_string: &EncryptedString,
    secret_password: &secrecy::SecretString,
) -> Option<secrecy::SecretString> {
    let secret_passcode_bytes = password_to_passcode(secret_password.expose_secret());
...
mod tests {
    ...
    fn test_encrypt_decrypt() {
        ...
        let encrypted_string =
            encrypt_symmetric_with_password(&secret_string, &secret_password).unwrap();
        let decrypted_string =
            decrypt_symmetric_with_password(&encrypted_string, &secret_password).unwrap();
        assert_eq!(
            decrypted_string.expose_secret(),
