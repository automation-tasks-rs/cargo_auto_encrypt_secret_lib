...
pub trait SshContextTrait {
    fn get_token_and_encrypt(&self) -> crate::EncryptedString;
...
pub fn encrypt_with_ssh_interactive_save_file(ssh_context: &mut impl SshContextTrait, identity_file_path: &camino::Utf8Path, encrypted_string_file_path: &camino::Utf8Path) {
    ...
    ssh_context.sign_with_ssh_agent_or_identity_file(&identity_file_path_expanded, &seed_bytes_not_a_secret);
    let encrypted_text = ssh_context.get_token_and_encrypt();
